...
export async function POST(request: Request) {
  ...
  // Security Check: Verify that the user owns the account by checking their description
  ...
  if (!description.includes(expectedCode)) {
    return NextResponse.json(
      { error: `Verification failed. Please add "${expectedCode}" to your Roblox profile's 'About' section and try again.` },
      { status: 403 }
    );
  }
